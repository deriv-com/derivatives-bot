...
function transformTradingTimesData(tradingTimesData: any): any {
    ...
    const validKeys = Object.keys(tradingTimesData || {}).filter(symbol => {
        ...
        if (!isValidSymbol) {
            console.warn(`[SmartCharts Adapter] Filtering out invalid symbol key: "${symbol}"`);
            return false;
        }
            ...
            if (symbolData.is_open_all_day) {
                ...
                symbolData.times.forEach((timeSession: any) => {
                    if (timeSession && timeSession.open && timeSession.close) {
                        ...
                        console.warn(`[SmartCharts Adapter] Invalid time session for ${symbol}:`, timeSession);
                    }
                });
            } else {
                console.warn(
                    `[SmartCharts Adapter] No valid times array for ${symbol}. Available properties:`,
    ...
    return {
        ...
        async getActiveSymbols(): Promise<any> {
            try {
                ...
                if (!Array.isArray(activeSymbols)) {
                    console.warn('No active symbols available from ApiHelpers');
                    return [];
                }
                ...
                try {
                    ...
                    console.error('Fallback active symbols retrieval failed:', fallbackError);
                }
        ...
        async getTradingTimes(): Promise<any> {
            try {
                ...
                if (!apiHelpers) {
                    console.error('[SmartCharts Adapter] ApiHelpers instance not available');
                    return {};
                }

                if (!apiHelpers.trading_times) {
                    console.error('[SmartCharts Adapter] trading_times not available on ApiHelpers instance');
                    return {};
                }
                ...
                if (!tradingTimesData || typeof tradingTimesData !== 'object') {
                    console.warn('[SmartCharts Adapter] No trading times data available, trying fallback...');
                ...
                console.error('[SmartCharts Adapter] Error getting trading times:', error);
                console.error(
                    '[SmartCharts Adapter] Error stack:',
